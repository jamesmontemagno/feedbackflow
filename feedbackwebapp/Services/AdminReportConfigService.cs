    ...
    public async Task<List<AdminReportConfigModel>> GetAllConfigsAsync()
    {
        try
        {
            var request = new HttpRequestMessage(HttpMethod.Get, 
                $"{_baseUrl}/api/admin/reports?code={Uri.EscapeDataString(_functionsKey)}");
    ...
    public async Task<AdminReportConfigModel> UpdateConfigAsync(AdminReportConfigModel config)
    {
        try
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Put, 
                $"{_baseUrl}/api/admin/reports/{Uri.EscapeDataString(config.Id)}?code={Uri.EscapeDataString(_functionsKey)}")
            {
                Content = content
            };
            await _headerService.AddAuthenticationHeadersAsync(request);
    ...
    public async Task<bool> DeleteConfigAsync(string id)
    {
        try
        {
            var request = new HttpRequestMessage(HttpMethod.Delete, 
                $"{_baseUrl}/api/admin/reports/{Uri.EscapeDataString(id)}?code={Uri.EscapeDataString(_functionsKey)}");
            await _headerService.AddAuthenticationHeadersAsync(request);
