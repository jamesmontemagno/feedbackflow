...
public class EmailService : IEmailService
{
    ...
    public async Task<EmailDeliveryStatus> SendWeeklyReportEmailAsync(WeeklyReportEmailModel emailModel)
    {
        try
        {
            if (!IsValidEmailAddress(emailModel.RecipientEmail))
            {
                return new EmailDeliveryStatus
                {
                    OperationId = string.Empty,
            ...
            var template = GenerateWeeklyReportEmailTemplate(emailModel);
            
            var emailMessage = new EmailMessage(
                senderAddress: _senderEmail,
                recipientAddress: emailModel.RecipientEmail,
            ...
            _logger.LogInformation("Sending {ReportType} report email to {Email} with subject '{Subject}'", 
                emailModel.IsAdminReport ? "admin" : "weekly", emailModel.RecipientEmail, template.Subject);
        ...
        {
            _logger.LogError(ex, "Failed to send weekly report email to {Email}", emailModel.RecipientEmail);
...
";
    ...
    private EmailTemplate GenerateWeeklyReportEmailTemplate(WeeklyReportEmailModel emailModel)
    {
        var reportTypeLabel = emailModel.IsAdminReport ? "Admin" : "Weekly";
        var subject = $"📊 {reportTypeLabel} Report: {emailModel.ReportTitle}";
        
        var reportSummary = string.Empty;
        var detailedContent = string.Empty;
        
        if (emailModel.ReportData != null)
        {
            reportSummary = "Report generated successfully";
                
            // Create a simplified view of the report data
            detailedContent = $@"
                <div style='background: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;'>
                    <h3>📈 Report Details</h3>
                    <p><strong>Source:</strong> {emailModel.ReportData.Source}</p>
                    <p><strong>Generated:</strong> {emailModel.ReportData.GeneratedAt:MMM dd, yyyy 'at' HH:mm} UTC</p>
                    <p><strong>Threads:</strong> {emailModel.ReportData.ThreadCount}</p>
                    <p><strong>Comments:</strong> {emailModel.ReportData.CommentCount}</p>
                </div>";
        }
...
<head>
    ...
    <style>
        ...
        .btn {{ display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px 30px; text-decoration: none; border-radius: 8px; font-weight: 600; margin: 10px 0; }}
...
<body>
    <div class='container'>
        ...
        <div class='content'>
            <p>Hi {emailModel.RecipientName}!</p>
            
            <p>Your {reportTypeLabel.ToLower()} report <strong>{emailModel.ReportTitle}</strong> is ready.</p>
            
            {detailedContent}
            
            <p style='margin-top: 30px;'>
                <a href='{WebUrlHelper.BuildUrl(_configuration, "/reports")}' class='btn'>View Report Dashboard</a>
            </p>
            
            {(emailModel.IsAdminReport ? "<p><em>This is an automated admin report generated by FeedbackFlow.</em></p>" : "")}
        </div>
        <div class='footer'>
            <p>Generated by <strong>FeedbackFlow</strong> 🥰</p>
            <p><small>Report period: {emailModel.WeekStartDate:MMM dd} - {emailModel.WeekEndDate:MMM dd, yyyy}</small></p>
        </div>
    </div>
</body>
</html>";
        ...
        var plainTextContent = $@"
{reportTypeLabel} FeedbackFlow Report

Hi {emailModel.RecipientName}!

Your {reportTypeLabel.ToLower()} report '{emailModel.ReportTitle}' is ready.

{reportSummary}

Report period: {emailModel.WeekStartDate:MMM dd} - {emailModel.WeekEndDate:MMM dd, yyyy}

View your reports: {WebUrlHelper.BuildUrl(_configuration, "/reports")}

---
Generated by FeedbackFlow
{(emailModel.IsAdminReport ? "This is an automated admin report." : "")}
";
