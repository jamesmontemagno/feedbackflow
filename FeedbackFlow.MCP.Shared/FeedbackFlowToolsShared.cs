...
public sealed class FeedbackFlowToolsShared
{
        ...
        [Description("Output mode (optional): AnalysisOnly=0 (markdown), CommentsOnly=1 (JSON), AnalysisAndComments=2 (combined JSON)")] AnalysisType? type = null)
    {
        ...
        return await ExecuteApiRequestAsync("api/AutoAnalyze", queryParams);
    }

    [McpServerTool, Description("Fetch only raw normalized comments (JSON) from a feedback source without AI analysis.")]
    public async Task<string> GetComments(
        [Description("The URL to fetch comments from (GitHub issue/discussion, YouTube video, Reddit post, HN item, etc.)")] string url,
        ...
        [Description("Force regeneration of cached report (default: false)")] bool force = false)
    {
        ...
        return await ExecuteApiRequestAsync("api/RedditReport", queryParams);
    }
        ...
        [Description("Force regeneration of cached report (default: false)")] bool force = false)
    {
        ...
        return await ExecuteApiRequestAsync("api/GitHubIssuesReport", queryParams);
    }
    ...
    {
        var queryParams = $"subreddit={Uri.EscapeDataString(subreddit)}&force={force}";
        return await ExecuteApiRequestAsync("api/RedditReportSummary", queryParams);
    }
}